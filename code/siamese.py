...
def data_loader():
    datasets, word_to_i_map = load_swbd_labelled(data_dir, min_count)
...
def get_batch(train_x, train_y, achor_xs, achor_ys, S):
    ...
    for s in range(batch_size):
        ...
        same_xs[s] = train_x[train_y == achor_y][
            torch.randint(n_achor_y, size=(1,), dtype=torch.long, device=cuda0)
        ]
...
def train(ntokens, train_x, train_y, train_ldr):
    ...
    for batch_idx, (achor_xs, achor_ys) in enumerate(train_ldr):
        achor_xs, same_xs, diff_xs, diff_ys = get_batch(
            train_x, train_y, achor_xs, achor_ys, S
        )
        optimizer.zero_grad()
        loss = net(achor_xs, same_xs, diff_xs)
        loss.backward()
